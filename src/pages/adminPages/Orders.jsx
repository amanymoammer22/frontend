    ...
    <div className="p-4 md:p-6 bg-[#f6e5cf] min-h-screen">
      ...
      <form onSubmit={onSearch} className="flex flex-wrap items-center gap-2 mb-3">
        <input
          ...
          className="flex-1 min-w-[220px] bg-white rounded-lg px-3 py-2 border"
        />
        <select
          value={status}
          onChange={(e) => {
      ...
      <div className="rounded-2xl bg-white/70 shadow-sm p-3 md:p-4">
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
          <div className="grid grid-cols-9 gap-2 bg-gray-100 text-gray-600 text-sm font-medium px-3 py-2">
            <div>Order ID</div>
            <div>Customer</div>
            <div>Email</div>
            <div>Phone</div>
          ...
          {loading ? (
            ...
            orders.map((o) => (
              <div
                key={o._id}
                className="grid grid-cols-9 gap-2 px-3 py-2 border-t text-sm items-center"
              >
                ...
                <div className="flex justify-center gap-2">
                  <a
                    ...
                    className="px-2 py-1 rounded-md bg-blue-600/10 text-blue-600 hover:bg-blue-600/20"
                    title="View"
                  >
                    🔍
                  </a>
        ...
        <div className="flex justify-center gap-2 mt-4">
          <button
            ...
            disabled={page <= 1}
            className="px-3 py-1 rounded border disabled:opacity-50 bg-white hover:bg-gray-50"
          >
            Prev
          </button>
          ...
          <button
            onClick={() => fetchOrders(Math.min(pages, page + 1))}
            disabled={page >= pages}
            className="px-3 py-1 rounded border disabled:opacity-50 bg-white hover:bg-gray-50"
          >
            Next
